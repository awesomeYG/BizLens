...
interface IMSectionNavProps {
  current: "settings" | "notifications" | "alerts";
}
...
const ITEMS: Array<{ key: IMSectionNavProps["current"]; href: string; label: string }> = [
  ...
  { key: "alerts", href: "/alerts", label: "告警与通知" },
];
  ...
  return (
    <nav className="mb-6">
      <div className="inline-flex flex-wrap gap-1 rounded-xl border border-zinc-800/50 bg-zinc-900/70 p-1">
        {ITEMS.map((item) => {
          const active = item.key === current || (item.key === "alerts" && (pathname === "/alerts" || pathname.startsWith("/alerts/")));
          return (
            <Link
              key={item.key}
              href={item.href}
