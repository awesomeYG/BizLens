...
function getStatusTag(status: string): { text: string; className: string } {
  ...
  if (["ready", "active", "completed", "available", "success"].includes(normalizedStatus)) {
    return { text: "可用", className: "badge-success" };
  }
  if (["processing", "pending", "parsing", "uploading"].includes(normalizedStatus)) {
    return { text: "处理中", className: "badge-warning" };
  }
  if (["failed", "error", "invalid"].includes(normalizedStatus)) {
    return { text: "异常", className: "badge-error" };
  }
  return { text: status || "未知", className: "badge-neutral" };
}

function isFileReady(status: string): boolean {
  const s = status.toLowerCase();
  return ["ready", "active", "completed", "available", "success"].includes(s);
}
      ...
      <div className="glass-card rounded-xl p-4 mb-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          <div className="relative flex-1">
            <svg className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
