...
3. 说"生成数据大屏"创建可视化`
    ...
    // 定义 send_im_message 工具（仅在真正的数据异常时使用）
    const tools: OpenAI.Chat.ChatCompletionTool[] = [
      {
        type: "function",
        function: {
          name: "send_im_message",
          description: `发送即时消息到 IM 平台（钉钉、飞书、企业微信等）。
...
如果你不确定是否应该发送，不要调用此工具。`,
          parameters: {
            ...
            properties: {
              platform: {
                type: "string",
                description: "目标 IM 平台，取值：dingtalk（钉钉）、feishu（飞书）、wecom（企业微信）、slack、telegram、discord。仅在确认存在真实数据异常时才指定。",
