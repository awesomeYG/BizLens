...
type DataSourceHandler struct {
	...
	aiFindingService  *service.AIFindingService
	semanticService   *service.SemanticModelService
...
// SetAIFindingService 设置 AI 发现服务（可选）
func (h *DataSourceHandler) SetAIFindingService(svc *service.AIFindingService) {
	h.aiFindingService = svc
}

// SetSemanticService 设置语义模型服务（可选）
func (h *DataSourceHandler) SetSemanticService(svc *service.SemanticModelService) {
	h.semanticService = svc
}

// SetDashboardService 设置大屏服务（可选）
func (h *DataSourceHandler) SetDashboardService(svc *service.DashboardService) {
...
func (h *DataSourceHandler) CreateDataSource(w http.ResponseWriter, r *http.Request) {
	...
	// 自动触发 AI 发现（如果服务已设置）
	if h.aiFindingService != nil && ds.SchemaInfo != "" {
		go func() {
			_ = h.aiFindingService.AutoDiscover(tenantID, ds.ID)
		}()
	}

	// 自动构建语义缓存（如果服务已设置）
	if h.semanticService != nil && ds.SchemaInfo != "" {
		go func() {
			_ = h.semanticService.BuildSemanticCache(tenantID, ds.ID)
		}()
	}

	// 自动生成大屏（如果服务已设置）
	if h.dashboardService != nil && ds.SchemaInfo != "" {
		go func() {
			_, _ = h.dashboardService.AutoGenerateDashboard(tenantID, ds.ID, ds.Name+" - AI 大屏")
		}()
	}
...
func (h *DataSourceHandler) GetDataSourceSchema(w http.ResponseWriter, r *http.Request) {
	...
	if ds.SchemaInfo == "" {
		writeError(w, http.StatusNotFound, "暂无 Schema 信息")
		return
	}

	schema, err := model.DeserializeSchemaInfo(ds.SchemaInfo)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "解析 Schema 失败")
		return
	}
